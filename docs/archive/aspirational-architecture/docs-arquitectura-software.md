> **⚠️ Documento histórico, no implementado.** Esta es una propuesta de arquitectura aspiracional (NestJS, Mapbox, Isar, WebRTC) que nunca se construyó. **No refleja el código actual.** La arquitectura vigente está en [`docs/03-ARQUITECTURA_FRONTEND.md`](../../03-ARQUITECTURA_FRONTEND.md) y [`docs/05-ARQUITECTURA_BACKEND.md`](../../05-ARQUITECTURA_BACKEND.md). Se conserva aquí solo como referencia histórica.
